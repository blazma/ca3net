...
def run_simulation(wmx_PC_E, STDP_mode, w_PC_I, w_BC_E, w_BC_I, w_PC_MF, wmx_mult, rate_MF, cue, save, seed, verbose=True):
    """
...
if __name__ == "__main__":
    ...
    params = np.genfromtxt("gamma_hof_sym_0.5.csv", delimiter=",", names=True)
    for idx in range(len(params)):
        ...
        SM_PC, SM_BC, RM_PC, RM_BC, selection, StateM_PC, StateM_BC = run_simulation(wmx_PC_E, STDP_mode, w_PC_I_, w_BC_E_,
                                                                                     w_BC_I_, w_PC_MF_, wmx_mult_, rate_MF_,
                                                                                     cue=cue, save=save, seed=seed,
                                                                                     verbose=verbose)
        ...
        result_vrs = ["multiplier", "np.nan", "mean_rate_PC", "mean_rate_BC",
                      "avg_ripple_freq_PC", "ripple_power_PC", "avg_ripple_freq_BC",
                      "ripple_power_BC", "avg_ripple_freq_LFP", "ripple_power_LFP",
                      "avg_gamma_freq_PC", "gamma_power_PC", "avg_gamma_freq_BC",
                      "gamma_power_BC", "avg_gamma_freq_LFP", "gamma_power_LFP",
        ...
        with open(os.path.join(top10_output_dir, "results.txt"), "w") as results_file:
            for i in range(len(results)):
                results_file.writelines([result_vrs[idx], "=", str(results[idx]), "\n"])

        print("# copying figures")
        figures_dir = os.path.join(base_path, "figures")
        subprocess.call(['cp', '-a', figures_dir, top10_output_dir])
