    ...
    def evaluate_with_lists(self, individual, verbose=False, plots=False):
        ...
        try:
            ...
            if SM_PC.num_spikes > 0 and SM_BC.num_spikes > 0:  # check if there is any activity
                ...
                # look for significant gamma peak close to 50 Hz
                gamma_peakE = 2 * np.exp(-1/2*(avg_gamma_freq_PC-50.)**2/10**2) if not np.isnan(avg_gamma_freq_PC) else 0.
                gamma_peakI = np.exp(-1/2*(avg_gamma_freq_BC-50.)**2/10**2) if not np.isnan(avg_gamma_freq_BC) else 0.
                ...
                gamma_ratioE = np.clip(gamma_power_PC/subgamma_power_PC, 0., 5.)
                gamma_ratioI = np.clip(gamma_power_BC/subgamma_power_BC, 0., 5.)
