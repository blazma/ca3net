...
def run_simulation(wmx_PC_E_, w_PC_I_, w_BC_E_, w_BC_I_, w_PC_MF_, rate_MF_, save, seed, verbose=True):
    """
    ...
    MF = PoissonGroup(nPCs, rate_MF_)
    C_PC_MF = Synapses(MF, PCs, on_pre="x_ampaMF+=norm_PC_MF*w_PC_MF_")
    C_PC_MF.connect(j="i")

    # weight matrix used here
    C_PC_E = Synapses(PCs, PCs, "w_exc:1", on_pre="x_ampa+=norm_PC_E*w_exc", delay=delay_PC_E)
    nonzero_weights = np.nonzero(wmx_PC_E_)
    C_PC_E.connect(i=nonzero_weights[0], j=nonzero_weights[1])
    C_PC_E.w_exc = wmx_PC_E_[nonzero_weights].flatten()
    del wmx_PC_E_

    C_PC_I = Synapses(BCs, PCs, on_pre="x_gaba+=norm_PC_I*w_PC_I_", delay=delay_PC_I)
    C_PC_I.connect(p=connection_prob_BC)

    C_BC_E = Synapses(PCs, BCs, on_pre="x_ampa+=norm_BC_E*w_BC_E_", delay=delay_BC_E)
    C_BC_E.connect(p=connection_prob_PC)

    C_BC_I = Synapses(BCs, BCs, on_pre="x_gaba+=norm_BC_I*w_BC_I_", delay=delay_BC_I)
...
if __name__ == "__main__":
    ...
    for idx in range(len(params)):
        ...
        top10_output_dir = os.path.join(base_path, "top10", str(idx))
        if not os.path.exists(top10_output_dir):
            os.mkdir(top10_output_dir)
        with open(os.path.join(top10_output_dir, "params.txt"), "w") as params_file:
            params_file.writelines(["w_PC_I=", str(w_PC_I_), "\n",
                                    "w_BC_E=", str(w_BC_E_), "\n",
                                    "w_BC_I=", str(w_BC_I_), "\n",
                                    "wmx_mult=", str(wmx_mult_), "\n",
                                    "w_PC_MF=", str(w_PC_MF_), "\n",
                                    "rate_MF=", str(rate_MF_), "\n"])

        start_scope()
        f_in = "wmx_sym_0.5_linear.pkl"
        wmx_PC_E = load_wmx(os.path.join(base_path, "files", f_in)) * wmx_mult_ * 1e9  # *1e9 nS conversion
        SM_PC, SM_BC, RM_PC, RM_BC, selection, StateM_PC, StateM_BC = run_simulation(wmx_PC_E, w_PC_I_, w_BC_E_,
                                                                                     w_BC_I_, w_PC_MF_, rate_MF_,
                                                                                     save=save, seed=seed,
                                                                                     verbose=verbose)
        ...
        with open(os.path.join(top10_output_dir, "results.txt"), "w") as results_file:
